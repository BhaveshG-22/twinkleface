...
export default function PortraitsContent({ isCollapsed = false }: PortraitsContentProps) {
  const [portraitPrompts, setPortraitPrompts] = useState<PromptData[]>([])
  const [loading, setLoading] = useState(true)

  // Fetch prompts from JSON file and filter portrait-related ones
  useEffect(() => {
    const fetchPrompts = async () => {
      try {
        ...
        // Portrait-related keywords to filter by
        const portraitKeywords = ['portrait', 'fashion', 'editorial', 'street', 'headshot', 'studio', 'lifestyle']

        // Filter prompts that are portrait-related and add preview images + metadata
        const filteredPrompts = data.prompts
          .filter((prompt: { title: string; prompt: string }) => {
            ...
            return portraitKeywords.some(keyword =>
              titleLower.includes(keyword) || promptLower.includes(keyword)
            )
          })
          .map((prompt: { id: number; title: string; prompt: string; variables: string[] }, index: number) => {
            // Generate preview images, categories, difficulties, and tags based on prompt content
            const previewImages = [
            ...
            const titleLower = prompt.title.toLowerCase()
            if (titleLower.includes('fashion')) category = 'fashion'
            else if (titleLower.includes('editorial')) category = 'editorial'
            else if (titleLower.includes('street')) category = 'street'
            else if (titleLower.includes('studio') || titleLower.includes('spotlight')) category = 'studio'
            else if (titleLower.includes('lifestyle') || titleLower.includes('casual')) category = 'lifestyle'
            else if (titleLower.includes('artistic') || titleLower.includes('creative') || titleLower.includes('abstract')) category = 'artistic'
            ...
            const tags: string[] = []
            if (titleLower.includes('fashion')) tags.push('fashion')
            if (titleLower.includes('black') || titleLower.includes('bw')) tags.push('black-white')
            if (titleLower.includes('street')) tags.push('street')
            if (titleLower.includes('editorial')) tags.push('editorial')
            if (titleLower.includes('studio')) tags.push('studio')
            if (titleLower.includes('professional')) tags.push('professional')
            if (titleLower.includes('lifestyle')) tags.push('lifestyle')
            if (titleLower.includes('artistic')) tags.push('artistic')
            if (titleLower.includes('creative')) tags.push('creative')
            if (titleLower.includes('cinematic')) tags.push('cinematic')
            if (titleLower.includes('moody')) tags.push('moody')
        ...
        setPortraitPrompts(filteredPrompts)
      } catch (error) {
    ...
    <div className="flex flex-col gap-6 pb-12 w-full">
      <div className={`flex flex-col gap-6 transition-all duration-300 ${isCollapsed ? 'px-8' : 'px-6'}`}>
        ...
        <div className="text-center space-y-6 max-w-4xl mx-auto mt-8">
          ...
          <h1 className="text-4xl font-bold text-white mb-2">Portrait Styles</h1>
          <p className="text-gray-400 text-lg max-w-2xl mx-auto">
            Perfect for LinkedIn, business cards, or personal branding.
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6 mt-6">
            ...
            <div className="flex items-center gap-2 text-purple-400">
              <span className="text-sm font-medium">🎨 {loading ? '...' : portraitPrompts.length} unique styles</span>
            </div>
          </div>
        </div>
            ...
            Array.from({ length: 6 }).map((_, index) => (
              <Card key={index} className="bg-gray-900/50 border-gray-800/50 overflow-hidden animate-pulse">
                <CardContent className="p-0">
                  <div className="h-[32rem] bg-gray-800"></div>
        ...
        {/* Empty State */}
        {!loading && portraitPrompts.length === 0 && (
          <div className="text-center py-12">
            <div className="text-6xl mb-4">🔍</div>
            <h3 className="text-xl font-semibold text-white mb-2">No portraits found</h3>
            <p className="text-gray-400">Check back later for new portrait styles</p>
          </div>
        )}
        ...
        <div className="text-center mt-16 py-12 border-t border-gray-800/50">
          <div className="space-y-4 max-w-2xl mx-auto">
            ...
            <p className="text-gray-400 text-lg">
              Choose any portrait style above and watch as AI transforms your photos into stunning professional portraits in minutes.
            </p>
