...
export default function CartoonContent({ isCollapsed = false }: CartoonContentProps) {
  const [cartoonPrompts, setCartoonPrompts] = useState<PromptData[]>([])
  const [loading, setLoading] = useState(true)

  // Fetch prompts from JSON file and filter cartoon-related ones
  useEffect(() => {
    const fetchPrompts = async () => {
      try {
        ...
        // Cartoon-related keywords to filter by
        const cartoonKeywords = ['cartoon', 'animated', 'comic', 'illustration', 'stylized', 'character', 'disney', 'pixar', 'cute', 'fun', 'playful', 'colorful', 'bright', 'cheerful', 'friendly', 'whimsical', 'fantasy', 'magical', 'adventure', 'animated series', 'cartoon style', 'comic book', 'graphic', 'vector', 'flat design']
        
        // Filter prompts that are cartoon-related and add preview images + metadata
        const filteredPrompts = data.prompts
          .filter((prompt: { title: string; prompt: string }) => {
            ...
            // Check for cartoon keywords or fun/playful terms
            return cartoonKeywords.some(keyword => 
              combined.includes(keyword)
            ) || 
            // Also include prompts that mention creative/fun terms
            ['creative', 'artistic', 'vibrant', 'bold', 'expressive', 'imaginative', 'whimsical'].some(term => 
              combined.includes(term)
            )
          })
          .map((prompt: { id: number; title: string; prompt: string; variables: string[] }, index: number) => {
            // Generate preview images for cartoon styles - fun and colorful
            const previewImages = [
            ...
            const titleLower = prompt.title.toLowerCase()
            if (titleLower.includes('disney') || titleLower.includes('pixar')) category = 'disney-style'
            else if (titleLower.includes('comic') || titleLower.includes('comic book')) category = 'comic-book'
            else if (titleLower.includes('cute') || titleLower.includes('kawaii')) category = 'cute'
            else if (titleLower.includes('fantasy') || titleLower.includes('magical')) category = 'fantasy'
            else if (titleLower.includes('adventure') || titleLower.includes('hero')) category = 'adventure'
            else if (titleLower.includes('colorful') || titleLower.includes('bright')) category = 'colorful'
            else if (titleLower.includes('flat') || titleLower.includes('vector')) category = 'flat-design'
            ...
            const tags: string[] = []
            if (titleLower.includes('disney') || titleLower.includes('pixar')) tags.push('disney-style')
            if (titleLower.includes('comic')) tags.push('comic-book')
            if (titleLower.includes('cute')) tags.push('cute')
            if (titleLower.includes('fantasy')) tags.push('fantasy')
            if (titleLower.includes('adventure')) tags.push('adventure')
            if (titleLower.includes('colorful')) tags.push('colorful')
            if (titleLower.includes('bright')) tags.push('bright')
            if (titleLower.includes('fun')) tags.push('fun')
            if (titleLower.includes('playful')) tags.push('playful')
            if (titleLower.includes('whimsical')) tags.push('whimsical')
            if (titleLower.includes('cheerful')) tags.push('cheerful')
            if (titleLower.includes('flat')) tags.push('flat-design')
        ...
        setCartoonPrompts(filteredPrompts)
      } catch (error) {
      ...
      <div className={`flex flex-col gap-6 transition-all duration-300 ${isCollapsed ? 'px-8' : 'px-6'}`}>
        ...
        <div className="text-center space-y-6 max-w-4xl mx-auto mt-8">
          <div className="inline-flex items-center gap-2 bg-gradient-to-r from-orange-500/10 to-red-500/10 border border-orange-500/20 rounded-full px-4 py-2">
            ...
            <span className="text-orange-300 text-sm font-medium">CARTOON STYLES</span>
          </div>
          <h1 className="text-4xl font-bold text-white mb-2">Cartoon Styles</h1>
          <p className="text-gray-400 text-lg max-w-2xl mx-auto">
            Transform your photos into fun, colorful cartoon characters and illustrations. Perfect for social media, avatars, and creative projects.
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6 mt-6">
            <div className="flex items-center gap-2 text-green-400">
              ...
              <span className="text-sm font-medium">20,000+ cartoon avatars created</span>
            </div>
            <div className="hidden sm:block w-1 h-1 bg-gray-600 rounded-full"></div>
            <div className="flex items-center gap-2 text-orange-400">
              <span className="text-sm font-medium">⚡ Disney quality</span>
            </div>
            <div className="hidden sm:block w-1 h-1 bg-gray-600 rounded-full"></div>
            <div className="flex items-center gap-2 text-red-400">
              <span className="text-sm font-medium">🎭 {loading ? '...' : cartoonPrompts.length} unique styles</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-8 w-full px-4">
          {loading ? (
            ...
            cartoonPrompts.map((prompt, index) => {
              const isCompareCard = index % 2 === 0; // Every even index gets a Compare component
              
              if (isCompareCard) {
                return (
                  <Card key={prompt.id} className="group bg-gray-900/20 border-gray-700/30 hover:border-orange-500/50 transition-all duration-300 overflow-hidden hover:scale-[1.02] hover:-translate-y-1 shadow-lg hover:shadow-2xl hover:shadow-orange-500/20 backdrop-blur-sm">
                    <CardContent className="p-0">
                    ...
                    <CardContent className="p-0">
                        ...
                        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center">
                          <div className="transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                            <Button className="bg-white/95 text-black hover:bg-white font-bold shadow-2xl px-6 py-3 text-lg">
                              <span className="text-orange-600 mr-2 text-xl">🎭</span> Try This Style
                            </Button>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/80 via-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-4 group-hover:translate-y-0">
                        ...
                        <div className="flex items-center justify-between text-sm text-gray-300">
                          ...
                          <span className="capitalize text-orange-400 font-medium">{prompt.category}</span>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
        ...
        {/* Empty State */}
        {!loading && cartoonPrompts.length === 0 && (
          <div className="text-center py-12">
            <div className="text-6xl mb-4">🎭</div>
            <h3 className="text-xl font-semibold text-white mb-2">No cartoon styles found</h3>
            <p className="text-gray-400">Check back later for new cartoon styles</p>
          </div>
        )}
        ...
        <div className="text-center mt-16 py-12 border-t border-gray-800/50">
          <div className="space-y-4 max-w-2xl mx-auto">
            ...
            <h3 className="text-3xl font-bold text-white">
              Create Your Cartoon Avatar Today
            </h3>
            <p className="text-gray-400 text-lg">
              Choose any cartoon style above and watch as AI transforms your photos into fun, colorful characters in minutes.
            </p>
