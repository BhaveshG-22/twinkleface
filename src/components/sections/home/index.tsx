      ...
      <div className="grid grid-cols-4 gap-4 h-full overflow-hidden">
        {Array.from({ length: columns }).map((_, columnIndex) => (
          <div key={columnIndex} className="relative">
            ...
            >
              {layoutItems
                .filter((item: { column: number }) => item.column === columnIndex)
                .map((item: { index: number; src: string; height: number; category: string; title: string; style: string; column: number }, index: number) => (
                  <motion.div
